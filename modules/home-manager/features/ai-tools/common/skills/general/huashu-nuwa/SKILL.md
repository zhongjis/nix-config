...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查 `.opencode/skills/` 目录）
...
**来源A：本地已有Skill**
扫描 `.opencode/skills/*-perspective/` 目录，读取每个SKILL.md的description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何perspective skill），跳过此步，只从来源B推荐。
...
```
.opencode/skills/[person-name]-perspective/
├── SKILL.md                          # 最终产物
...
Phase 1启动前，**主动扫描 `.opencode/skills/` 目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入 `.opencode/skills/[person-name]-perspective/SKILL.md`。
