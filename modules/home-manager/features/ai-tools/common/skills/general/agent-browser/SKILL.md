...
The CLI uses Chrome/Chromium via CDP directly. Make sure `agent-browser` is available in your environment, then run `agent-browser install` to download Chrome. Run `agent-browser upgrade` to update to the latest version.
...
**Requirements:** macOS with Xcode, plus Appium with the `xcuitest` driver installed.
